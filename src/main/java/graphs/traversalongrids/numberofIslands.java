...
public class numberofIslands {
    ...
    private static void dfs(char[][] grid, int i, int j) {
        ...
        if (i < 0 || i >= grid.length || j < 0 || j >= grid[0].length || grid[i][j] != '1') {  // IF CELL
            // IS 2 THEN RETURN. ELSE MARK IT 2
            return;   // MAKE SURE grid[i][j] != '1' IS ALWAYS IN THE END. WHY?
            // Coz if its in front and i= -1 then grid [-1][]  will give indexOutOfBound Exception
        }
