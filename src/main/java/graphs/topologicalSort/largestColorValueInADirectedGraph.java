...
public class largestColorValueInADirectedGraph {
    ...
    public static int largestPathValue(String colors, List<List<Integer>> edges) {// har node ka konsa color hoga vo iss string ke index pe mil jaega
        ...
        while (!q.isEmpty()) {
            ...
            //count of f , f is our node, usss node me jo color hai vo colors naam ki string se aajaega and usme se agar 'a' subtract karenge to it will be mapped to an index
            //from 0 to 25, so jo bhi color nikal ke aaega +1 hojaega
            // agar ye nai karna to end me firse for loop chalega to get the max of all the count of colors String array
            ...
            for (int nbr : graph.get(f)) {
                ...
                //iterate over colors 0 to 25 and Im saying uss nbr(cnt[nbr]) ka vo particular color(cnt[j]) should be max of jo usme already saved hua hai(Math.max(cnt[nbr][j],)  v/s jo mera vo particular node se aya
                for (int j = 0; j < 26; j++) {
                    cnt[nbr][j] = Math.max(cnt[nbr][j], cnt[f][j]);
                }
            }
        }
