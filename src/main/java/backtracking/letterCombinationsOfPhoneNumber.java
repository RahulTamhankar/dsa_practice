...
public class letterCombinationsOfPhoneNumber {
    ...
    public static List<String> letterCombinations(String digits) {
        ...
        // Call the helper method to generate all combinations
        generateCombinations(result, digits.toCharArray(), "", map);

        return result;
    }

    private static void generateCombinations(List<String> result, char[] digits, String current, String[] map) {
        // If the current combination's length matches the number of digits, add it to the result list
        ...
        int digit = digits[index] - '0';  // Convert the char to its corresponding digit value

        // Loop through all the letters mapped to the current digit
        for (char letter : map[digit].toCharArray()) {
            // Recursively build the combination by adding the letter and continuing the process
            generateCombinations(result, digits, current + letter, map);
        }
    }
